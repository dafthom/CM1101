...
def print_menu(exits):
    ...
    for i in directions:
        try: print_menu_line(i, exit_leads_to(exits, i))
        except:
            pass
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while True:
        ...
        choice = input(">")
        # Normalise the input
        normalisedText = normalise_input(choice)
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice

        splitText = normalisedText.split(" ")
        enteredDirection = ""
        go = False
        for i in splitText:
            if(i == "go"):
                go = True
            if(i == "north" or i == "south" or i == "east" or i == "west"):
                enteredDirection = i
                break
        if(go and is_valid_exit(exits, enteredDirection)):
